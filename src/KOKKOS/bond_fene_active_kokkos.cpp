...
#include "bond_fene_active_kokkos.h"
...
BondFENEACTIVEKokkos<DeviceType>::BondFENEACTIVEKokkos(LAMMPS *lmp) : BondFENEACTIVE(lmp)
{
  kokkosable = 1;
...
template<class DeviceType>
BondFENEACTIVEKokkos<DeviceType>::~BondFENEACTIVEKokkos()
{
  if (!copymode) {
...
template<class DeviceType>
void BondFENEACTIVEKokkos<DeviceType>::compute(int eflag_in, int vflag_in)
{
  eflag = eflag_in;
  vflag = vflag_in;
  ...
  if (evflag) {
    if (newton_bond) {
      Kokkos::parallel_reduce(Kokkos::RangePolicy<DeviceType, TagBondFENEACTIVECompute<1,1> >(0,nbondlist),*this,ev);
    } else {
      Kokkos::parallel_reduce(Kokkos::RangePolicy<DeviceType, TagBondFENEACTIVECompute<0,1> >(0,nbondlist),*this,ev);
    }
  } else {
    if (newton_bond) {
      Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType, TagBondFENEACTIVECompute<1,0> >(0,nbondlist),*this);
    } else {
      Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType, TagBondFENEACTIVECompute<0,0> >(0,nbondlist),*this);
    }
  }
  ...
  if (h_flag() == 1)
    ...
    error->one(FLERR,"Bad FENEACTIVE bond");
...
void BondFENEACTIVEKokkos<DeviceType>::operator()(TagBondFENEACTIVECompute<NEWTON_BOND,EVFLAG>, const int &n, EV_FLOAT& ev) const {
  ...
  const F_FLOAT sqrtrsq = sqrt(rsq);

  if (NEWTON_BOND || i1 < nlocal) {
    a_f(i1,0) += delx*(fbond+(d_Fa[type]/sqrtrsq));
    a_f(i1,1) += dely*(fbond+(d_Fa[type]/sqrtrsq));
    a_f(i1,2) += delz*(fbond+(d_Fa[type]/sqrtrsq));
  }

  if (NEWTON_BOND || i2 < nlocal) {
    a_f(i2,0) -= delx*(fbond-(d_Fa[type]/sqrtrsq));
    a_f(i2,1) -= dely*(fbond-(d_Fa[type]/sqrtrsq));
    a_f(i2,2) -= delz*(fbond-(d_Fa[type]/sqrtrsq));
  }
...
template<int NEWTON_BOND, int EVFLAG>
KOKKOS_INLINE_FUNCTION
void BondFENEACTIVEKokkos<DeviceType>::operator()(TagBondFENEACTIVECompute<NEWTON_BOND,EVFLAG>, const int &n) const {
  EV_FLOAT ev;
  this->template operator()<NEWTON_BOND,EVFLAG>(TagBondFENEACTIVECompute<NEWTON_BOND,EVFLAG>(), n, ev);
}

/* ---------------------------------------------------------------------- */

template<class DeviceType>
void BondFENEACTIVEKokkos<DeviceType>::allocate()
{
  BondFENEACTIVE::allocate();

  int n = atom->nbondtypes;
  k_k = DAT::tdual_ffloat_1d("BondFeneactive::k",n+1);
  k_r0 = DAT::tdual_ffloat_1d("BondFeneactive::r0",n+1);
  k_epsilon = DAT::tdual_ffloat_1d("BondFeneactive::epsilon",n+1);
  k_sigma = DAT::tdual_ffloat_1d("BondFeneactive::sigma",n+1);
  k_Fa = DAT::tdual_ffloat_1d("BondFeneactive::Fa",n+1);

  d_k = k_k.template view<DeviceType>();
  d_r0 = k_r0.template view<DeviceType>();
  d_epsilon = k_epsilon.template view<DeviceType>();
  d_sigma = k_sigma.template view<DeviceType>();
  d_Fa = k_Fa.template view<DeviceType>();
}
...
template<class DeviceType>
void BondFENEACTIVEKokkos<DeviceType>::coeff(int narg, char **arg)
{
  BondFENEACTIVE::coeff(narg, arg);
...
template<class DeviceType>
void BondFENEACTIVEKokkos<DeviceType>::read_restart(FILE *fp)
{
  BondFENEACTIVE::read_restart(fp);
...
KOKKOS_INLINE_FUNCTION
void BondFENEACTIVEKokkos<DeviceType>::ev_tally(EV_FLOAT &ev, const int &i, const int &j,
...
namespace LAMMPS_NS {
template class BondFENEACTIVEKokkos<LMPDeviceType>;
#ifdef LMP_KOKKOS_GPU
template class BondFENEACTIVEKokkos<LMPHostType>;
#endif
}
