...
#include "bond_fene_alpha_kokkos.h"
...
BondFENEALPHAKokkos<DeviceType>::BondFENEALPHAKokkos(LAMMPS *lmp) : BondFENEALPHA(lmp)
{
...
template<class DeviceType>
BondFENEALPHAKokkos<DeviceType>::~BondFENEALPHAKokkos()
{
  if (!copymode) {
...
template<class DeviceType>
void BondFENEALPHAKokkos<DeviceType>::compute(int eflag_in, int vflag_in)
{
  eflag = eflag_in;
  vflag = vflag_in;
  ...
  if (evflag) {
    if (newton_bond) {
      Kokkos::parallel_reduce(Kokkos::RangePolicy<DeviceType, TagBondFENEALPHACompute<1,1> >(0,nbondlist),*this,ev);
    } else {
      Kokkos::parallel_reduce(Kokkos::RangePolicy<DeviceType, TagBondFENEALPHACompute<0,1> >(0,nbondlist),*this,ev);
    }
  } else {
    if (newton_bond) {
      Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType, TagBondFENEALPHACompute<1,0> >(0,nbondlist),*this);
    } else {
      Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType, TagBondFENEALPHACompute<0,0> >(0,nbondlist),*this);
    }
  }
  ...
  if (h_warning_flag())
    error->warning(FLERR,"FENEALPHA bond too long");

  k_error_flag.template modify<DeviceType>();
  k_error_flag.template sync<LMPHostType>();
  if (h_error_flag())
    error->one(FLERR,"Bad FENEALPHA bond");
...
void BondFENEALPHAKokkos<DeviceType>::operator()(TagBondFENEALPHACompute<NEWTON_BOND,EVFLAG>, const int &n, EV_FLOAT& ev) const {
  ...
  F_FLOAT sr6 = 0.0;
  F_FLOAT sigma2 = d_sigma[type]*d_sigma[type];
  if (rsq < MY_CUBEROOT2*sigma2) {
    const F_FLOAT sr2 = sigma2/rsq;
    sr6 = sr2*sr2*sr2;
    fbond += 48.0*d_epsilon[type]*sr6*(sr6-0.5)/rsq;
  } else if (rsq<2.25*sigma2) {
    // force from solvophobic alpha potential
    const F_FLOAT sr2 = rsq/sigma2 ;
    fbond += d_alpha[type]*d_epsilon[type]*MY_GAMMA*sin((sr2-9./4.)*MY_GAMMA) ;
  }
  ...
  if (eflag) {
    ...
    if (rsq < MY_CUBEROOT2*sigma2) {
      ebond += 4.0*d_epsilon[type]*sr6*(sr6-1.0) + (1.0 - d_alpha[type]) * d_epsilon[type];
    } else if (rsq < 2.25*sigma2) {
      const F_FLOAT sr2 = rsq/sigma2 ;
      ebond += 0.5 * d_alpha[type] * d_epsilon[type] * ( cos( (sr2 - 9./4.) * MY_GAMMA ) - 1. ) ;
    }
  }
...
template<int NEWTON_BOND, int EVFLAG>
KOKKOS_INLINE_FUNCTION
void BondFENEALPHAKokkos<DeviceType>::operator()(TagBondFENEALPHACompute<NEWTON_BOND,EVFLAG>, const int &n) const {
  EV_FLOAT ev;
  this->template operator()<NEWTON_BOND,EVFLAG>(TagBondFENEALPHACompute<NEWTON_BOND,EVFLAG>(), n, ev);
}

/* ---------------------------------------------------------------------- */

template<class DeviceType>
void BondFENEALPHAKokkos<DeviceType>::allocate()
{
  BondFENEALPHA::allocate();

  int n = atom->nbondtypes;
  k_k = DAT::tdual_ffloat_1d("BondFeneAlpha::k",n+1);
  k_r0 = DAT::tdual_ffloat_1d("BondFeneAlpha::r0",n+1);
  k_epsilon = DAT::tdual_ffloat_1d("BondFeneAlpha::epsilon",n+1);
  k_sigma = DAT::tdual_ffloat_1d("BondFeneAlpha::sigma",n+1);
  k_alpha = DAT::tdual_ffloat_1d("BondFeneAlpha::alpha",n+1);
...
template<class DeviceType>
void BondFENEALPHAKokkos<DeviceType>::coeff(int narg, char **arg)
{
  BondFENEALPHA::coeff(narg, arg);
  ...
  k_alpha.template modify<LMPHostType>();
}
...
template<class DeviceType>
void BondFENEALPHAKokkos<DeviceType>::read_restart(FILE *fp)
{
  BondFENEALPHA::read_restart(fp);
...
KOKKOS_INLINE_FUNCTION
void BondFENEALPHAKokkos<DeviceType>::ev_tally(EV_FLOAT &ev, const int &i, const int &j,
...
namespace LAMMPS_NS {
template class BondFENEALPHAKokkos<LMPDeviceType>;
#ifdef LMP_KOKKOS_GPU
template class BondFENEALPHAKokkos<LMPHostType>;
#endif
}
