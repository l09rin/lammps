...
void BondFENEALPHA::compute(int eflag, int vflag)
{
  ...
  for (n = 0; n < nbondlist; n++) {
    ...
    if (rlogarg < 0.1) {
      ...
      if (rlogarg <= -3.0) error->one(FLERR,"Bad FENEALPHA bond");
      rlogarg = 0.1;
    }
    ...
    } else if (rsq < 2.25 * sigma[type] * sigma[type]) {
      // force from solvophobic alpha potential
      sr2 = rsq / sigma[type] / sigma[type];
      fbond += alpha[type] * epsilon[type] * MY_GAMMA * sin( (sr2 - 9./4.) * MY_GAMMA ) ;
    }
    ...
    if (eflag) {
      ...
      if (rsq < MY_CUBEROOT2 * sigma[type] * sigma[type]) {
        ebond += 4.0 * epsilon[type] * sr6 * (sr6 - 1.0) + (1.0 - alpha[type]) * epsilon[type];
      } else if (rsq < 2.25 * sigma[type] * sigma[type]) {
	sr2 = rsq / sigma[type] / sigma[type];
	ebond += 0.5 * alpha[type] * epsilon[type] * ( cos( (sr2 - 9./4.) * MY_GAMMA ) - 1. ) ;
      }
    }
...
void BondFENEALPHA::coeff(int narg, char **arg)
{
...
void BondFENEALPHA::init_style()
{
  ...
  if (force->special_lj[1] != 0.0 || force->special_lj[2] != 1.0 ||
    ...
    if (comm->me == 0)
      error->warning(FLERR,"Use special bonds = 0,1,1 with bond style fene/alpha");
  }
}

/* ---------------------------------------------------------------------- */

double BondFENEALPHA::equilibrium_distance(int i)
{
  return 0.97*sigma[i];
}
...
void BondFENEALPHA::write_restart(FILE *fp)
{
...
void BondFENEALPHA::read_restart(FILE *fp)
{
  allocate();
...
void BondFENEALPHA::write_data(FILE *fp)
{
  for (int i = 1; i <= atom->nbondtypes; i++)
    fprintf(fp,"%d %g %g %g %g %g\n",i,k[i],r0[i],epsilon[i],sigma[i], alpha[i]);
}

/* ---------------------------------------------------------------------- */

double BondFENEALPHA::single(int type, double rsq, int /*i*/, int /*j*/,
                        double &fforce)
{
  ...
  if (rlogarg < 0.1) {
    error->warning(FLERR,"FENEALPHA bond too long: {} {:.8}",
  ...
  if (rsq < MY_CUBEROOT2 * sigma[type] * sigma[type]) {
    ...
    sr2 = sigma[type] * sigma[type] / rsq;
    sr6 = sr2 * sr2 * sr2;
    eng += 4.0 * epsilon[type] * sr6 * (sr6 - 1.0) + (1.0 - alpha[type]) * epsilon[type];
    fforce += 48.0 * epsilon[type] * sr6 * (sr6 - 0.5) / rsq;
  } else if (rsq < 2.25 * sigma[type] * sigma[type]) {
    // force from solvophobic alpha potential
    double sr2 = rsq / sigma[type] / sigma[type];
    eng += 0.5 * alpha[type] * epsilon[type] * ( cos( (sr2 - 9./4.) * MY_GAMMA ) - 1. ) ;
    fforce += alpha[type] * epsilon[type] * MY_GAMMA * sin( (sr2 - 9./4.) * MY_GAMMA ) ;
  }
...
void *BondFENEALPHA::extract(const char *str, int &dim)
{
  dim = 1;
